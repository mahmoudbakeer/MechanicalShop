            ...
            while (startDate < endDate)
            {
                foreach (Spot spot in spots)
                {
                    ...
                    while (occupiedMinutes < minOccupancy && currentTime.TimeOfDay < closeTime)
                    {
                        ...
                        var duration = selectedTask.Sum(st => (int)st.EstimatedDuration);
                        ...
                        var availableVehicle = vehicles
                            .Where(v =>
                                !generatedWorkOrders.Any(w =>
                                    w.VehicleId == v.Id
                                    && w.StartedAtUtc.Date == startAt.Date
                                    && w.StartedAtUtc < endAt
                                    && w.EndAtUtc > startAt
                                )
                            )
                            .OrderBy(_ => Guid.NewGuid())
